...
export class CriterionController {
  ...
  @Put("/:criterionId")
  update(@Param("projectId") projectId: number, @Param("criterionId") criterionId: number, @Body() newSpec: CriterionSpec) {
    if (!criteria.has(projectId) || criteria.get(projectId)!.length <= criterionId)
      return
